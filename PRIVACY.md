...
`mcp-supertoinette` is a read-only client for [Supertoinette](https://www.supertoinette.com). It runs on your
...
**One host is contacted: `www.supertoinette.com`.** Nothing else.
...
| A `User-Agent`         | `mcp-supertoinette/<version> (+https://github.com/smeet666/mcp-supertoinette)`, so the site can reach a person about the traffic it receives. |
| Your IP address        | Sent by your network to any host you contact, as with any web request.                                                                        |

Your requests reach Supertoinette. What is done with them there is governed by that site's own privacy policy, which this project does not control.
...
becomes of them. `STO_LOG_LEVEL` governs how much is written and defaults to `error`. These lines stay on your machine.

## The settings that change any of this

| Variable           | What it changes                                                                                                  |
| ------------------ | ---------------------------------------------------------------------------------------------------------------- |
| `STO_USER_AGENT`   | Adds your own identifier in front of this project's, which stays appended so the site can always reach a person. |
| `STO_CACHE_TTL_MS` | How long an answer is held in memory. `0` turns the cache off.                                                   |
| `STO_LOG_LEVEL`    | How much is written to stderr.                                                                                   |
...
Open an issue on [the repository](https://github.com/smeet666/mcp-supertoinette/issues). For something exploitable,
...
`mcp-supertoinette` est un client en lecture seule pour [Supertoinette](https://www.supertoinette.com). Il
...
**Un seul hôte est joint : `www.supertoinette.com`.** Rien d'autre.
...
| Un `User-Agent`   | `mcp-supertoinette/<version> (+https://github.com/smeet666/mcp-supertoinette)`, pour que le site puisse joindre une personne au sujet du trafic qu'il reçoit. |
| Votre adresse IP  | Transmise par votre réseau à tout hôte que vous joignez, comme pour n'importe quelle requête web.                                                             |

Vos requêtes atteignent Supertoinette. Ce qui en est fait là-bas relève de la politique de confidentialité de ce site, que ce projet ne contrôle pas.
...
qu'ils deviennent. `STO_LOG_LEVEL` règle leur quantité et vaut `error` par défaut. Ces lignes restent sur votre machine.

## Les réglages qui changent tout cela

| Variable           | Ce qu'elle change                                                                                                        |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------ |
| `STO_USER_AGENT`   | Ajoute votre identifiant devant celui du projet, qui reste accolé pour que le site puisse toujours joindre une personne. |
| `STO_CACHE_TTL_MS` | Combien de temps une réponse est gardée en mémoire. `0` éteint le cache.                                                 |
| `STO_LOG_LEVEL`    | La quantité écrite sur stderr.                                                                                           |
...
Ouvrez une issue sur [le dépôt](https://github.com/smeet666/mcp-supertoinette/issues). Pour quelque chose
