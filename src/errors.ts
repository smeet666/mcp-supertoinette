...
export class SupertoinetteError extends Error {
  readonly code: ErrorCode;
  readonly details: ErrorDetails;

  constructor(code: ErrorCode, message: string, details: ErrorDetails = {}) {
    super(message, details.cause === undefined ? undefined : { cause: details.cause });
    this.name = "SupertoinetteError";
    this.code = code;
    this.details = details;
  }
}

export const notFound = (message: string, details?: ErrorDetails): SupertoinetteError =>
  new SupertoinetteError("not_found", message, details);

export const invalidInput = (message: string, hint?: string): SupertoinetteError =>
  new SupertoinetteError("invalid_input", message, hint ? { hint } : {});

export const rateLimited = (message: string, details?: ErrorDetails): SupertoinetteError =>
  new SupertoinetteError("rate_limited", message, {
    hint: "Wait a moment and ask again. This says nothing about whether the recipe exists.",
    ...details,
  });

export const parseFailure = (message: string, details?: ErrorDetails): SupertoinetteError =>
  new SupertoinetteError("parse_failure", message, {
    hint: `The site may have changed how it answers. Please report this at ${REPO_URL}/issues with the arguments you used.`,
    ...details,
  });

export const networkError = (message: string, details?: ErrorDetails): SupertoinetteError =>
  new SupertoinetteError("network_error", message, details);

export const timeout = (message: string, details?: ErrorDetails): SupertoinetteError =>
  new SupertoinetteError("timeout", message, details);
